...
bool GameSound::InitializeDirectSound(){
	HRESULT result;
...
//
void GameSound::FinalizeDIrectSound(){
	if (m_primaryBuffer)
	{
...
//
bool GameSound::LoadWaveFile(const char* fileName, IDirectSoundBuffer8** secondaryBuffer){
	int error;
	FILE* filePtr;
	unsigned int count;
...
//
void GameSound::FinalizeWaveFile(IDirectSoundBuffer8** secondaryBuffer){
	if (*secondaryBuffer)
	{
...
//
bool GameSound::PlayWaveFile(DWORD isLoop){
	HRESULT result;
...
//
void GameSound::SetValume(const float valume){
	m_secondaryBuffer1->SetVolume(valume);
}

//
void GameSound::SetSpeed(long speed){
